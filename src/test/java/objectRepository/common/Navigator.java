...
import objectRepository.CustomisedReportsTMO.TrafficAnalysisReportByPMN;
...
public class Navigator {
	...
	public void traverseMenu_VerifyPageTitle(PagesTitle PagesTitle,Object... elementArr) {
		Perform_traverseMenu_VerifyPageTitle( PagesTitle.title, elementArr);
	}
	...
	public TrafficAnalysisReportByPMN to_TrafficAnalysisReportByPMN() {
		// DO NOT REMOVE THIS METHOD, IT IS REQUIRED COZ NAVIGATION MENU DOES
		// NOT HAVE LINK FOR Traffic Analysis Report by PMN Page  
		if (navigateToPageNumber(TrafficAnalysisReportByPMN.title, 22)) {
			return new TrafficAnalysisReportByPMN();
		}
		return null;
	}

	public Navigator to_IOTRONHomePage() {
		NGCHomePage ngcHome = new NGCHomePage();
		com.click(ngcHome.link_IOTRON);
		if (com.verifyPageTitle(IOTRONHomePage.title, false)) {
			CustomReporter.report(STATUS.PASS, "'<span style='color: black;font-style: italic;font-weight: bold;'>"+ com.getTitle() + "</span>' page is displayed <br/>"+com.getCurrentUrl());
			return this;
		}
		CustomReporter.report(STATUS.FAIL, "'<span style='color: black;font-style: italic;font-weight: bold;'>"+ IOTRONHomePage.title + "</span>' page is NOT displayed but '<b>"+com.getTitle()+"</b>' is getting displayed <br/>"+com.getCurrentUrl());
		Assert.fail("to_IOTRONHomePage() method failed");
		return null;
	}
	
	public Navigator to_NGCHomePage() {
		NGCHomePage ngcHome = new NGCHomePage();
		com.click(ngcHome.link_FCH);
		if (com.verifyPageTitle(NGCHomePage.title, false)) {
			CustomReporter.report(STATUS.PASS,
					"Navigation complete <span style='color: black;font-style: italic;font-weight: bold;'>"
							+ NGCHomePage.title + "</span> <br/>" + com.getCurrentUrl());
			return this;
		}
		CustomReporter.report(STATUS.FAIL,
				"Navigation failed <span style='color: black;font-style: italic;font-weight: bold;'>"
						+ NGCHomePage.title + "</span> <br/>" + com.getCurrentUrl());
		return null;
	}

	public NavigatorDT to_DealTrackerHome() {
		IOTRONHomePage i= new IOTRONHomePage();
		traverseMenu_VerifyPageTitle( PagesTitle.DealTracker, i.link_Forecasting, i.link_Forecasting_DealTracker);
		return new NavigatorDT();
	}
